    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        this.filterdata.values =
          dialogRef.componentInstance.filterResult.values;
      } else {
        this.loaimathangManagementService.fetch();
      }
    });
    // dialogRef.componentInstance.filterResult.subscribe((filteredData: LoaiMatHangModel[]) => {
    //   this.LoaiMatHangsResult = filteredData;
