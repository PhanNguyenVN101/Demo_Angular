  ...
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.loadInitData();
    this.nhomtaisanManagementService.items$.subscribe((data) => {
      this.itemsnts = data;
    });
  }
  loadInitData() {
  ...
  Luu() {
    ...
    if (this.formGroup.valid) {
      ...
      const existsma = this.itemsnts.some(
        (item) => item.MaNhom === model.MaNhom
      );
      if (existsma) {
      ...
      const exists = this.itemsnts.some(
        (item) => item.TenNhom.toLowerCase() === model.TenNhom.toLowerCase()
      );
      if (exists) {
