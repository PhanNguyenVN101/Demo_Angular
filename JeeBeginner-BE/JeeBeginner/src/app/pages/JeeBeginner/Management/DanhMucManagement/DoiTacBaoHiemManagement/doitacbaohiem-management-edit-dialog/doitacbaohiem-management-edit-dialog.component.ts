  ...
  implements OnInit, OnDestroy
{
  ...
  getTranslation(): string {
    return this.isExpanded
      ? this.translateService.instant("COMMOM.THUNHO")
      : this.translateService.instant("COMMOM.PHONGTO");
  }
  getTitle() {
  ...
  ngOnInit(): void {
    ...
    this.loadInitData();
    this.doitacbaohiemManagementService.items$.subscribe((data) => {
      this.itemsdtbh = data;
    });
  }
  loadInitData() {
  ...
  Luu() {
    ...
    if (this.formGroup.valid) {
      ...
      const exists = this.itemsdtbh.some(
        (item) => item.TenDonVi.toLowerCase() === model.TenDonVi.toLowerCase()
      );
      if (exists) {
