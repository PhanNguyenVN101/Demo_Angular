...
export class MatHangManagementEditDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.mathangManagementService.items$.subscribe((data) => {
      this.itemsmh = data;
    });
  }
  ...
  LuuVaThemMoi() {
    if (this.formGroup.valid) {
      ...
      const existsma = this.itemsmh.some(
        (item) => item.MaHang === model.MaHang
      );
      if (existsma) {
      ...
      const exists = this.itemsmh.some(
        (item) =>
          item.TenMatHang.toLowerCase() === model.TenMatHang.toLowerCase()
      );
      if (exists) {
